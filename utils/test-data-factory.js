...
class PersonDataFactory extends BaseFactory {
  constructor() {
    ...
    this.firstNames = [
      ...
      'Emma', 'Liam', 'Olivia', 'Noah', 'Sophia', 'Mason', 'Isabella', 'William',
      'Ava', 'James', 'Charlotte', 'Benjamin', 'Mia', 'Lucas', 'Amelia', 'Henry'
    ];
    ...
    this.domains = [
      'example.com', 'test.org', 'demo.net', 'sample.co', 'testing.io',
      'automation.dev', 'qa.test', 'playwright.test'
    ];
  ...
  email(options = {}) {
    ...
    const baseEmail = prefix || `${this.firstName().toLowerCase()}.${this.lastName().toLowerCase()}`;
  ...
  person(options = {}) {
    ...
    return {
      ...
      email: this.email({ ...options, prefix: `${firstName.toLowerCase()}.${lastName.toLowerCase()}` }),
...
class FormDataFactory extends BaseFactory {
  constructor() {
    ...
    this.inquiryTypes = [
      'General Inquiry', 'Support Request', 'Sales Question', 'Technical Issue',
      'Feature Request', 'Bug Report', 'Partnership Opportunity', 'Consultation'
    ];

    this.subjects = [
      'Request for Information', 'Product Demo Request', 'Technical Support Needed',
      'Pricing Question', 'Custom Solution Inquiry', 'Website Feedback',
      'Service Integration', 'Partnership Discussion'
    ];
    ...
    this.companyTypes = [
      'Software Development', 'E-commerce', 'Healthcare', 'Education', 'Finance',
      'Marketing Agency', 'Non-profit', 'Government', 'Manufacturing', 'Retail'
    ];

    this.companyNames = [
      'TechCorp Solutions', 'Digital Innovations LLC', 'Global Systems Inc',
      'NextGen Technologies', 'Smart Solutions Group', 'Advanced Analytics Co',
      'Future Forward Inc', 'Dynamic Systems LLC', 'Integrated Solutions Group'
    ];
  }
  ...
  company() {
    return {
      ...
      size: this.randomChoice(['Small (1-10)', 'Medium (11-50)', 'Large (51-200)', 'Enterprise (200+)']),
  ...
  newsletterSignup(options = {}) {
    ...
    return {
      ...
      firstName: person.firstName,
      lastName: person.lastName,
      interests: this.randomChoice(['Technology', 'Business', 'Design', 'Marketing', 'Development']),
...
class WordPressDataFactory extends BaseFactory {
  constructor() {
    ...
    this.postTitles = [
      'Welcome to Our Website', 'Getting Started Guide', 'Latest Updates and Features',
      'Tips for Success', 'Common Questions Answered', 'Best Practices Guide',
      'Important Announcement', 'New Features Released', 'How-To Tutorial'
    ];

    this.categories = [
      'Announcements', 'Tutorials', 'News', 'Updates', 'Guides', 'Tips',
      'Features', 'Support', 'Documentation', 'Resources'
    ];

    this.tags = [
      'important', 'featured', 'new', 'updated', 'guide', 'tutorial',
      'tips', 'help', 'documentation', 'announcement'
    ];
  }
...
class NavigationDataFactory extends BaseFactory {
  constructor() {
    ...
    this.commonPages = [
      ...
      '/support', '/faq', '/privacy', '/terms', '/sitemap'
    ];

    this.ecommercePages = [
      '/shop', '/cart', '/checkout', '/account', '/orders', '/wishlist',
      '/products/category', '/products/featured', '/sale'
    ];

    this.blogPages = [
      '/blog', '/blog/category', '/blog/tag', '/blog/archive',
      '/blog/author', '/blog/search', '/blog/latest'
    ];
  }
